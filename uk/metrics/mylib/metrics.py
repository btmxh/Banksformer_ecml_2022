...
def compute_ngram_metrics(
    ...
    n_codes_unique = len(
        set(real_df[field].unique()).union(set(gen_df[field].unique()))
    )
    ...
    combo_df = pd.merge(
        real_ngrams, gen_ngrams, on="ngram", how="outer", suffixes=("_real", "_gen")
    ).fillna(0.0)
    ...
    combo_df["ps_real"] = combo_df["counts_real"] / (
        N_obs_real + N_possible_ngrams * pseudo_counts
    )
    combo_df["counts_gen"] += pseudo_counts
    combo_df["ps_gen"] = combo_df["counts_gen"] / (
        N_obs_gen + N_possible_ngrams * pseudo_counts
    )
    ...
    try:
        results = {
            ...
            "jac": distance.jaccard(
                combo_df["counts_real"] > 0, combo_df["counts_gen"] > 0
            ),
...
def create_ngramcount_df(df, n, field):
    gb = df.sort_values(by=["account_id", "datetime"]).groupby(
    ...
    df = pd.DataFrame.from_dict(counts, orient="index", columns=["counts"]).sort_values(
...
def compute_2d_categorical_cont_metrics(real_df, gen_df, field1):
    print("hi")
    f1_opts = set(real_df[field1].unique()).union(
        set(gen_df[field1].unique())
    )  # all unique options for field1 (in both dfs)
    ...
    for code_1 in f1_opts:
        ...
        for field in CONT_FIELDS:  # loop over continous fields
            ...
            for name, fn in CONTINUOUS_METRICS.items():  # loop over continous metrics
                to_add_r = p_r * fn(
                    vals_r, vals_g
                )  # expectation over R (check this makes sense)
                to_add_g = p_g * fn(
                    vals_r, vals_g
                )  # expectation over G (check this makes sense)

                kr = name + "_r"
                kg = name + "_g"

                cont_metric_results[field][kr] = (
                    cont_metric_results[field].get(kr, 0.0) + to_add_r
                )
                cont_metric_results[field][kg] = (
                    cont_metric_results[field].get(kg, 0.0) + to_add_g
                )

    return cont_metric_results


def compute_2d_categorical_metrics(
    real_df,
    gen_df,
    field1,
    field2,
    ...
    for code_1 in f1_opts:
        if plot:
            print(
                f"{field2.capitalize()}s of real transaction with {field1} =",
                code_1,
                f"{len(real_df[real_df[field1] == code_1]) / len(real_df) * 100:.2f}% of real data",
            )
            if plot_ecdfs:
                fig, ax = plt.subplots(
                    1, 3, figsize=(24, 7), gridspec_kw={"width_ratios": [3, 1, 1]}
                )
                #                 fig, ax = plt.subplots(1, 2, figsize=(20,7), gridspec_kw={'width_ratios': [3, 1]})

                make_barplot(
                    real_df[real_df[field1] == code_1],
                    gen_df[gen_df[field1] == code_1],
                    field=field2,
                    ax=ax[0],
                    **barplot_params,
                )
                ...
                plot_ecdf(
                    real_df[real_df[field1] == code_1]["raw_amount"],
                    plot_params={"label": "real"},
                    ax=ax[2],
                )
                plot_ecdf(
                    gen_df[gen_df[field1] == code_1]["raw_amount"],
                    plot_params={"label": "gen"},
                    ax=ax[2],
                )
                ...
                ax[1].legend()
                ax[1].set_title(
                    f"Hist of transaction amounts with \n {field1} = {code_1}"
                )
        ...
        for code_2 in f2_opts:
            cond_r = np.logical_and(
                real_df[field1] == code_1, real_df[field2] == code_2
            )
            cond_g = np.logical_and(gen_df[field1] == code_1, gen_df[field2] == code_2)

            p_r = (np.sum(cond_r) + pseudo_counts) / (
                len(cond_r) + pseudo_counts * n_opts_total
            )
            p_g = (np.sum(cond_g) + pseudo_counts) / (
                len(cond_g) + pseudo_counts * n_opts_total
            )
            p_m = (p_r + p_g) / 2.0
            ...
            if compute_cont_metrics:
                for field in CONT_FIELDS:  # loop over continous fields
                    ...
                    for (
                        name,
                        fn,
                    ) in CONTINUOUS_METRICS.items():  # loop over continous metrics
                        to_add_r = p_r * fn(
                            vals_r, vals_g
                        )  # expectation over R (check this makes sense)
                        to_add_g = p_g * fn(
...
def compute_all_metrics(real_data_loc, gen_data_loc):
    ...
    real_df = pd.read_csv(real_data_loc, parse_dates=["datetime"], date_format="mixed")
    preproc_df(real_df)

    gen_df = pd.read_csv(gen_data_loc, parse_dates=["date"], date_format="mixed")
    ...
    for n in [1, 3]:
        ...
        for code in CAT_CODE_FIELDS:
            ngram_res["code"][n][f"{code}"] = compute_ngram_metrics(
                real_df, gen_df, field=code, n=n, plot=False
            )

        ngram_res["tcode"][n] = compute_ngram_metrics(
            real_df, gen_df, field="tcode", n=n, plot=False
        )

        ngram_res["date"][n] = {}
        for code in CAT_DATE_FIELDS:
            ngram_res["date"][n][f"{code}"] = compute_ngram_metrics(
                real_df, gen_df, field=code, n=n, plot=False
            )
    ...
    for code in CAT_CODE_FIELDS:
        amount_codes_res["code"][f"{code}"] = compute_2d_categorical_cont_metrics(
            real_df, gen_df, code
        )

    amount_codes_res["tcode"] = compute_2d_categorical_cont_metrics(
        real_df, gen_df, code
    )
    ...
    for code in CAT_DATE_FIELDS:
        amount_codes_res["date"][f"{code}"] = compute_2d_categorical_cont_metrics(
            real_df, gen_df, code
        )
    ...
    for date_field in CAT_DATE_FIELDS:
        ...
        for code in CAT_CODE_FIELDS:
            amount_code_date_res["code"][date_field][code] = (
                compute_2d_categorical_metrics(
                    real_df, gen_df, code, date_field, plot=False, plot_ecdfs=False
                )
            )

        amount_code_date_res["tcode"][date_field] = compute_2d_categorical_metrics(
            real_df, gen_df, code, date_field, plot=False, plot_ecdfs=False
        )
